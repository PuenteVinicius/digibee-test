...
describe('useLevelManager', () => {
  ...
  const renderHookWithProps = (props: Partial<{ initialLevel: CreateLevels }> = {}) => {
  ...
  it('should accept custom initial level', () => {
    const { result } = renderHookWithProps({ initialLevel: CreateLevels.STEP_1 });
  ...
  it('should navigate to a new level and update history', () => {
    ...
    expect(result.current.currentLevel).toBe(CreateLevels.STEP_1);
    expect(result.current.history).toEqual([CreateLevels.MAIN, CreateLevels.STEP_1]);
